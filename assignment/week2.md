...
```
데이터가 겹치지 않도록 하기 위해 측정값을 사용합니다. 지역별 수익과 매출을 시각화할 때, 측정값을 사용하지 않으면 수익과 매출 부분이 따로 나오지 않고 하나의 그래프로 나타나므로 어떤 그래프가 수익을 나타내고 어떤 그래프가 매출을 나타내는지 한 눈에 알아보기 힘들다. "측정값 사진"
```
